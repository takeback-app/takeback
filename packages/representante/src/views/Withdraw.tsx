...
export function WithDrawPage() {
  ...
  const { data: withDrawOrders, isLoading } = useSWR<Withdraw[]>(
  ...
  async function onSubmit({ pixKey, moneyValue }: WithdrawCreateData) {
    ...
    try {
      await API.post('representative/withdraws', { pixKey, value })
      await mutate('representative/withdraws')
  ...
  async function cancelWithdraw(id: string) {
    ...
    try {
      const { data } = await API.post<{ message: string }>(
        `representative/withdraws/${id}/cancel`
      )
