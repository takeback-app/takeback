...
export function PaymentValue() {
  ...
  function setPaymentValue(value: string, rowValue: string) {
  ...
  function navigateToGetPassword() {
    ...
    if (Math.floor(valueReplaced * 100) > Math.floor(userData.balance * 100)) {
  ...
  return (
    <Layout>
      <Header variant="arrow" title="Pagar" goBack={goBack} />
        ...
        <VStack flex="1" justifyContent="space-between" pb="4" px="4">
          <VStack mt="4">
            ...
            <Text fontSize="md" color="gray.600" fontWeight="medium" mt="2">
              Saldo disponível: {masks.maskCurrency(userData.balance)}
            </Text>
